...
| 11. Language Selector widget + Language Content variants | Complete | KDNA_RC_Language_Selector_Widget (ARIA combobox, full Elementor styling for trigger/panel/options/arrow with hover/focus/selected states, reload or live-swap modes), `kdna-widgets` category registered, parallel "Language Content" repeater on the six existing widgets via additive base-class extension, per-item Restrict to Languages on Icon List, frontend.js variant swap with language-wins precedence, `window.kdnaRCRefreshLanguage()` live-swap entry point, custom DOM event `kdna-rc-language-changed`. |
| 12. Multilingual field types and matching widgets | Complete | Three new JetEngine field types (Multilingual Text, Image, WYSIWYG) targeting JetEngine 3.x with tabbed admin UI (one tab per language + Default, completion dot, flag icons), three matching Elementor Dynamic widgets (Field, Image, Link) under the kdna-widgets category with full Elementor styling, frontend.js multilingual swap pass + 50KB payload warning, anti-flicker rule extended to `.kdna-rc-mlf`, batched migration tool on the Tools tab to convert existing simple-text fields to their multilingual equivalents. |
| 13. JetEngine query/filter/search adapters and audit tool | Complete | KDNA_RC_Multilingual_Query_Helper rewrites meta_query clauses to match inside the serialised storage format. Adapters for JetSmartFilters (final-query rewrite + filter-option label translation), JetSearch (cross-language search with admin toggle), JetEngine Query Builder, and REST API (`Accept-Language`-aware response resolution with `?multilingual=raw` opt-out). Public `KDNA_RC::translate_query_args()` helper. Field Translation Audit tool on the Tools tab with completeness summary, per-post grid, and bulk-add-empty-slots action. README sections for Working with, Developer guide, Limitations, and SEO. |
| 14. Per-region and per-language URL routing | Complete | KDNA_RC_URL_Routing strips region/language/combined prefixes from REQUEST_URI before WordPress parses, supporting every permalink context (page, post, CPT, taxonomy, paginated archive, search). kdna_region + kdna_language registered as query vars. Snapshot-aware flush_rules_if_needed() auto-fires on regions / languages CRUD; manual Flush button on Tools tab. URL-prefix forces cookies via Stage 4 / Stage 10 detectors and 404s on stale slugs. Optional 301 bare→regional / bare→language redirects with crawler exemption (Googlebot, Bingbot, GPTBot, ClaudeBot, etc.). General-tab settings: canonical strategy radio (stored for Stage 15), redirect toggles, eligible-post-type whitelist. Per-post URL-preview meta box with copy buttons + "Test as visitor from" selector. Tools tab cache-bloat estimate. |
| 15. Yoast integration, hreflang, sitemap, regional SEO meta | Complete | KDNA_RC_SEO_Meta_Box adds a Default + per-region + per-language tabbed editor below Yoast on eligible post types, writing suffixed meta keys (_yoast_wpseo_title_au etc.). KDNA_RC_Yoast_Integration overrides Yoast's title / metadesc / OG / Twitter / focus / canonical filters with language-first resolution (region-first for canonical and schema). KDNA_RC_Yoast_MF_Variable_Resolver unwraps Stage 12 Multilingual Field values inside %%cf_*%% template variables. KDNA_RC_Hreflang emits x-default + alternate tags at wp_head priority 1 with Yoast Premium auto-defer. KDNA_RC_Yoast_Sitemap_Integration extends Yoast's sitemap (or generates a supplementary kdna-rc-sitemap.xml) with hreflang xhtml:link rows. KDNA_RC_Yoast_Schema_Integration overrides LocalBusiness / Organization address + phone per region. KDNA_RC_SEO_Health_Check Tools-tab scanner reports configuration findings with remediation hints. README sections for Multilingual SEO, Yoast integration, vs WPML, SEO checklist. |
